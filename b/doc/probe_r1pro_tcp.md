...
| 配置 | 现象 |
|---|---|
| `eef z=+0.06`（壳体/近腕侧，方向错误） | tracking 数字可能好看，但腕关节叠在物体上、手指伸开 |
| `eef z=-0.06`（手指抓取中心）+ 等权 `kp` | 人手姿态常不可达 → 腕关节顶死 → grip_site 被拖偏 |
| `eef z=-0.06` + **位置优先** `kp=[300,300,300,5,5,5]` | 位置误差可到 mm 级；夹爪对到人手 |

结论：TCP 必须在手指侧（−Z）；短 TCP 下必须 **位置优先**；不要用 `+0.06` 或 Robotiq `0.155` 假长。
...
```python
BASE_CFG = dict(
    r_pos=(-0.28, -0.28, 1.74), r_rot=(0.0, 0.0, np.pi / 2),  # 右臂
    l_pos=(-0.22, 0.35, 1.56), l_rot=(0.0, 0.0, -np.pi / 2),  # 左臂
)
```
...
| 项 | 值 |
|---|---|
| TCP | `eef pos="0 0 -0.06"`（手指侧） |
| 右基座 | `(-0.18, -0.40, 1.70)`, yaw `+π/2` |
| 左基座 | `(-0.15, 0.42, 1.55)`, yaw `-π/2` |
