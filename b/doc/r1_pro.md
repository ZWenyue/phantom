...
- `submodules/phantom-robosuite/robosuite/models/robots/manipulators/__init__.py` — 注册 R1ProRightArm
...
- `phantom/twin_bimanual_robot.py` — 支持 robot_name 为 list（左右臂使用不同模型）
...
- Pipeline 端到端可运行，约 88.5% 帧通过 tracking error 阈值（5cm）
- 渲染质量已修复，机械臂各部件正确连接
